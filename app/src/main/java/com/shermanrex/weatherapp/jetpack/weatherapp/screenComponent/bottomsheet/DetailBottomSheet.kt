...
import com.shermanrex.weatherapp.jetpack.weatherapp.screens.weatherPage.component.DetailGridItem
...
@Composable
private fun BottomSheetLayout(ForecastData: SevenDayForecastModel.Data) {
  ...
  Column(
	...
	Column(
	  ...
	  Text(text = ForecastData.validDate, fontWeight = FontWeight.Light)
	  Image(
		painter = painterResource(
		  id = iconFinder.getIconResource(
	  ...
	  Row {
		Text(text = buildAnnotatedString {
		  ...
		  withStyle(
			...
			block = { append(ForecastData.maxTemp.roundToInt().toString() + "°") }
		  )
		})
		Text(buildAnnotatedString {
		  ...
		  withStyle(
			...
			block = { append(ForecastData.minTemp.roundToInt().toString() + "°") }
		  )
		})
		Spacer(modifier = Modifier.height(10.dp))
	  }
	...
	LazyVerticalGrid(columns = GridCells.Fixed(2), Modifier.heightIn(max = 1000.dp), content = {
	  ...
	  item {
		DetailGridItem(
		  ...
		  value = ForecastData.windSpd.roundToInt().toString() + " m/s",
		  textColor = MaterialTheme.colorScheme.onSecondary,
		  backgroundColor = Color.Transparent
		)
	  }
